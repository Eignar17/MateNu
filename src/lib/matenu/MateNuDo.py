...
class MateNu ():
	...
	def __init__(self, applet):
		self.applet = applet
		self.applet.set_tooltip_text("MateNu")
		...
		proc = os.popen("""ps axo "%p,%a" | grep "MateNu.py" | grep -v grep|cut -d',' -f1""").read()
		procs = proc.split('\n')
		if len(procs) > 2:
			import wnck
			try:
...
applet_theme_logo = INSTALL_PREFIX + "/lib/matenu/graphics/" + "logo.png"

if __name__ == "__main__":
    awnlib.init_start(MateNu, {"name": applet_name,
        "short": "MateNu",
        "version": __version__,
