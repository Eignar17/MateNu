...
#sys.path.append(INSTALL_PREFIX + '/lib/matenu/') #is this causing the oafiid error???
...
class MateNu(mateapplet.Applet):
	...
	def __init__(self,applet,iid):
		...
		if Globals.FirstUse:
			os.system("/bin/sh -c " + INSTALL_PREFIX +"'/lib/"+Globals.appdirname+"/MateNu-Settings.py --welcome' &")
	...
	def remove_applet_border(self):
      		gtk.rc_parse_string ("""
	               ...
	               widget \"*.MateNu\" style \"MateNu-style\"
	               """)
	...
	def about_info(self,event,data=None):
		...
		os.system("/bin/sh -c " + INSTALL_PREFIX +"'/lib/"+Globals.appdirname+"/MateNu-Settings.py --about' &")
	...
	def properties(self,event,data=None):
		...
		os.system("/bin/sh -c '"+Globals.ProgramDirectory+"MateNu-Settings.py' &")
...
try:
	gobject.type_register(MateNu)
except:pass

# MateComponent factory of Menu
def MateNu_factory(applet, iid):
	# Start the applet
	app = MateNu(applet,iid)
	return True
...
if len(sys.argv) == 2:
	...
	if sys.argv[1] == "run-in-window" or sys.argv[1] == "run-in-console":
		...
		main_window.set_title("MateNu Testing Sandpit")
