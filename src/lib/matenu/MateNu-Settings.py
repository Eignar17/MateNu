...
class MateNuSettings:
	...
	def __init__(self):
		...
		self.window.set_title(_('MateNu Settings'))
	...
	def add_preferences_tab(self):
		...
		self.check8 = gtk.CheckButton(_('Activate Compiz blur effect on MateNu window'))
		self.Compiz_Blur =  utils.compiz_call('blur/screen0/focus_blur_match','get')
		if self.Compiz_Blur == '' :self.check8.set_sensitive(0)
		if self.Compiz_Blur.find('class=MateNu.py') != -1:
	...
	def add_about_tab(self):
		...
		self.label_app = gtk.Label("MateNu %s" % Globals.version)
		self.label_credits = gtk.Label("Consolidated menu for the MATE desktop.\nBy Helder Fraga aka Whise <helderfraga@gmail.com>\n Please Donate")
		...
		self.button9 = gtk.Button(_('MateNu Themes Specifications'))
	...
	def buttonpress(self,widget,id):
		if id == 'ok':
			...
			if utils.show_question(_('Menu needs to restart , restart now?')) is True:
				x = commands.getoutput("""ps axo "%p,%a" | grep "MateNu.py" | grep -v grep|cut -d',' -f1""")
				os.system('kill -9 ' + x)
			gtk.main_quit()
	...
	def button_clicked (self, widget, id):
		...
		if id == 'Donate':
			os.system('xdg-open http://mate-look.org/content/donate.php?content=93057 &')
		elif id == 'Download':
			os.system('xdg-open http://mate-look.org/index.php?xcontentmode=189 &')
		elif id == 'Specs':
			os.system('xdg-open http://launchpad.net/matenu/trunk/2.3/+download/MateNuThemeSpec.pdf &')
		elif id == 'Bug':
			os.system('xdg-open https://bugs.launchpad.net/matenu &')
		elif id == 'Translate':
			os.system('xdg-open https://translations.launchpad.net/matenu/trunk &')
		elif id == 'Homepage':
			os.system('xdg-open https://launchpad.net/matenu &')

		elif id == 'Credits':
			try:
				import mate.ui
				self.Applogo = gtk.gdk.pixbuf_new_from_file_at_size(Globals.Applogo,80,80)
				about = mate.ui.About(Globals.name,Globals.version,"GPL","Consolidated menu for the MATE desktop.",["Helder Fraga aka Whise <helderfraga@gmail.com>" , "https://launchpad.net/~matenu-team"],["https://launchpad.net/~matenu-team"],"https://translations.launchpad.net/matenu/trunk/+pots/matenu", self.Applogo)
				about.show()
			except:pass
	...
	def reload_themes(self,id):
		...
		if id == 'Menu':
			...
			themes = os.listdir('%s/share/matenu/Themes/Menu/' % INSTALL_PREFIX)
			...
			themes = os.listdir('%s/share/matenu/Themes/Button/' % INSTALL_PREFIX)
			themes.sort(key=str.upper)
			x = 0
			...
			themes = os.listdir('%s/share/matenu/Themes/Icon/' % INSTALL_PREFIX)
			themes.sort(key=str.upper)
			x = 0
			...
			themes = os.listdir('%s/share/matenu/Themes/Sound/' % INSTALL_PREFIX)
	...
	def install(self,filename):
		...
		if ext == 'bz2':
			tar_opts = 'xfj'
		if not os.path.isdir('/tmp/matenu/'):
			os.system('mkdir /tmp/matenu/')
		tmpdir = '/tmp/matenu/'
		...
		try:
			XMLBase = xml.dom.minidom.parse('/tmp/matenu/%s/themedata.xml' % name)
		except:
		...
		try:
			...
			os.system('rm -rf /tmp/matenu')
			return
		install_dir = '%s/share/matenu/Themes/%s/' % (INSTALL_PREFIX, type)
		...
		if type == 'Icon':
			...
			for i in Globals.MenuCairoSystemIcon:
				for n in IconFactory.Icontype:
					if os.path.exists('/tmp/matenu/%s/%s.%s' % (name, i, n)):
						x = 1
			...
			if x != 0:
				utils.show_message(_('This theme was created for a previous version of MateNu.\nThe theme was automaticly updated in order to work properly.'))

		elif type == 'Menu':
			x = 0
			fh = open('/tmp/matenu/%s/themedata.xml' % name)
			fhh = fh.read()
			fh.close()
			for i in Globals.MenuCairoSystemIcon:
				if fhh.find(i) != -1: x = 1
				...
				for n in IconFactory.Icontype:
					if os.path.exists('/tmp/matenu/' + name + '/' + i+  '.' + n):
						os.system(Globals.Settings['AdminRun'] +' ' + chr(34) + 'mv ' + "'"+ install_dir + name + '/' + i+  '.' + n + "'" + ' ' + "'" + install_dir + name + '/' + Globals.MenuCairoSystemIcon[i] +  '.' + n + "'" + chr(34))
						x = 1
			fi = open('/tmp/matenu/%s/themedata.xml' % name,'w')
			fi.write(fhh)
			fi.close()
			if x != 0:
				os.system(Globals.Settings['AdminRun'] +' ' + chr(34) + 'cp ' + "'"+ '/tmp/matenu/' + name + '/themedata.xml' + "'" + ' ' + "'" + install_dir + name + '/themedata.xml' + "'" + chr(34))
				utils.show_message(_('This theme was created for a previous version of MateNu.\nThe theme was automaticly updated in order to work properly.'))
	...
	def SaveSettings(self):
		...
		if self.check8.get_active():
			...
			if self.Compiz_Blur.find('class=MateNu.py') == -1:
				utils.compiz_call('blur/screen0/focus_blur_match','set',self.Compiz_Blur + "| class=MateNu.py")		
		else: 
			if self.Compiz_Blur.find('class=MateNu.py') != -1:
				utils.compiz_call('blur/screen0/focus_blur_match','set',self.Compiz_Blur.replace("class=MateNu.py",''))
